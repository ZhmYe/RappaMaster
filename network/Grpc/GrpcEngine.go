...
func (ge *GrpcEngine) Start(ctx context.Context) {
	...
	processRappaMasterServer := func() {
		...
		paradigm.Print("INFO", fmt.Sprintf("Coordinator gRPC server is running on :%d", ge.Port))
...
func (ge *GrpcEngine) connect(nodeId int) error {
	...
	if err != nil {
		return paradigm.RaiseError(paradigm.NetworkError, fmt.Sprintf("failed to connect to Node{%d}", nodeId), err)
	}
...
func (ge *GrpcEngine) GetConnection(nodeId int) (*grpc.ClientConn, error) {
	if nodeId >= len(ge.nodeAddress) {
		return nil, paradigm.RaiseError(paradigm.RuntimeError, fmt.Sprintf("nodeId %d out of range", nodeId), fmt.Errorf("%d >= %d", nodeId, len(ge.nodeAddress)))
	}
	ge.mu.RLock()
