...
func (d *Oracle) processQuery() {
	...
	for query := range d.channel.QueryChannel {
		LogWriter.Log("ORACLE", fmt.Sprintf("Receive a Query: %v", query.ToHttpJson()))
		switch query.(type) {
		case *Query.EvidencePreserveTaskIDQuery:
			...
			if task, exist := d.tasks[item.TaskID]; !exist {
				errorResponse := paradigm.NewErrorResponse(paradigm.ValueError, "Task does not exist in Oracle")
				item.SendResponse(errorResponse)
				paradigm.RaiseError(paradigm.ValueError, "Task does not exist in Oracle", false)
				continue
			} else {
		...
		case *Query.EvidencePreserveEpochIDQuery:
			...
			if epoch, exist := d.epochs[item.EpochID]; !exist {
				errorResponse := paradigm.NewErrorResponse(paradigm.ValueError, "Epoch does not exist in Oracle")
				item.SendResponse(errorResponse)
				paradigm.RaiseError(paradigm.ValueError, "Epoch does not exist in Oracle", false)
				continue
			} else {
		...
		case *Query.EvidencePreserveEpochTxQuery:
			...
			if ref.EpochID == -1 {
				errorResponse := paradigm.NewErrorResponse(paradigm.RuntimeError, "runtime error")
				item.SendResponse(errorResponse)
				paradigm.RaiseError(paradigm.RuntimeError, "Reference has no EpochID", false)
				continue
			}
			// 有ref但是没有epoch，要么是代码有问题，要么是交易对应的是taskProcess，此时epoch还没更新 TODO
			if epoch, exist := d.epochs[ref.EpochID]; !exist {
				errorResponse := paradigm.NewErrorResponse(paradigm.ValueError, "Epoch has not been update")
				item.SendResponse(errorResponse)
				paradigm.RaiseError(paradigm.RuntimeError, "Epoch has not been update", false)
			} else {
