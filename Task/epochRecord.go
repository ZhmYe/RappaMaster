...
type EpochRecord struct {
	...
	invalids  map[paradigm.SlotHash]paradigm.SlotCommitment // 在这个epoch里被检测出的问题slot, 节点可以根据这个删、改
}
...
func (r *EpochRecord) abort(slot paradigm.CommitSlotItem, reason paradigm.InvalidCommitType) {
	...
	if check() {
		...
		r.invalids[slot.SlotHash()] = slot.Commitment
	} else {
		// TODO
	}
...
func (r *EpochRecord) Refresh() {
	...
	r.invalids = make(map[paradigm.SlotHash]paradigm.SlotCommitment)
}
...
func NewEpochRecord() *EpochRecord {
	return &EpochRecord{
		...
		invalids:  make(map[paradigm.SlotHash]paradigm.SlotCommitment),
	}
}
